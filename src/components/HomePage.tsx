...
const GameCard = ({ title, type, slug, path }: { title: string; type: string; slug: string; path: string }) => {
  ...
  const getGameTypeInfo = (type: string) => {
    switch (type) {
      ...
      case "snake":
        return { name: "Snake Game", emoji: "🐍", gradient: "from-emerald-600 to-emerald-500" };
      default:
...
export const HomePage = () => {
  ...
  useEffect(() => {
    // Valid filter values
    const validFilters = ["all", "snake", "wordScramble", "wordSearch", "jigsaw", "memoryMatchIcon", "quizGame"];
