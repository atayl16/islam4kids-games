...
function calculateStreak(lastPlayed: string, previousStreak: number): number {
  const now = new Date();
  const last = new Date(lastPlayed);

  // Check if lastPlayed represents "never played" (Unix epoch)
  const neverPlayed = last.getTime() === 0;

  if (neverPlayed) {
    // First time playing - start with streak of 1
    return 1;
  }
...
export function useProgress() {
  ...
  const recordGameSession = useCallback((session: GameSession) => {
    setProgress((prev) => {
      ...
      // Calculate streak based on previous state
      const newStreak = calculateStreak(prev.lastPlayed, prev.streak);
